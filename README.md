...
- **CSRF write-guard**: write requests (POST/PUT/PATCH/DELETE) to `/api/*` require `Content-Type: application/json` (415 otherwise) and, whenever an `Origin`/`Referer` header is present, its host must match the request `Host` (403 otherwise). Header-less curl/scripts are unaffected; cross-site form/fetch writes riding the Access cookie are blocked.
