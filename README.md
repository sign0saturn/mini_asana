...
- **List column sorting**: sticky column-header row; click a header to cycle asc → desc → off (back to manual order; ↑/↓ marks the active column). Chinese-aware text comparison, undated always last; display-only (never writes order); defaults to due-date ascending on first visit, while explicit choices (including turning the sort off) are remembered per project; subtasks stay under their parent sorted among siblings, drag-reorder is disabled while a sort is active
...
- **Timeline**: drag bars to change start/due dates, drag edges to resize, dependency arrows (orthogonal routing with large rounded bends), multi-day and single-day bars, rubber-band marquee multi-select on blank-area drag (Shift adds; Esc/blank click clears) with batch shifting of the whole selection in one atomic `bulk_offset` write — Shift/Cmd/Ctrl-click multi-select works too (desktop); single-click on blank still opens the create dialog (the marquee replaces the old drag-to-create-a-range gesture), automatic task-name label placement; entering the view scrolls today into view (~18% from the left edge; manual scroll and drag-commit re-renders keep your position)
...
- **Cloudflare Access (primary, opt-in)**: when the app sits behind a cloudflared tunnel with Cloudflare Access in front, requests carrying the `Cf-Access-Authenticated-User-Email` header are authorized — but ONLY when you have configured an allowlist: set `MINI_ASANA_ACCESS_EMAIL` or write `data/access_email.txt`, and the header value must match that email (case-insensitive). Without a configured allowlist the header is ignored entirely, so a stripped or spoofed `Cf-*` header can never grant access by accident. This is safe because the origin listens on 127.0.0.1 and external traffic can only arrive through the tunnel (the edge scrubs client-supplied `Cf-*` headers). The logout button asks `GET /api/auth_mode` (public) which mode is in effect and ends the Access session at `/cdn-cgi/access/logout` when needed.
...
- **Request limits**: bodies are capped at 1 MiB (413 beyond, body never read), `Content-Length` must be a valid non-negative integer, the JSON top level must be an object; on any body-level failure the connection is closed so unread bytes cannot poison keep-alive.
- **CSRF write-guard**: write requests (POST/PUT/PATCH/DELETE) to `/api/*` that carry a body must use `Content-Type: application/json` (415 otherwise — a bodyless write without a Content-Type stays allowed for existing no-body operations) and, whenever an `Origin`/`Referer` header is present, its host must match the request `Host` (403 otherwise). All rejections close the connection (the unread body cannot poison keep-alive parsing). Header-less curl/scripts are unaffected; cross-site form/fetch writes riding the Access cookie are blocked.
- **Task field validation** (POST/PUT): strings with length caps (name ≤500 and non-empty, notes ≤20000, others ≤500), `completed` must be a real boolean, `start_on`/`due_on` must be `""` or a valid `YYYY-MM-DD`, `dependencies` must be a string list, `link` must start with `http://`/`https://` (the UI renders other protocols as plain text), and a task's start→due span may not exceed 3700 days. JSON `null` is accepted for every field except `name`/`completed` and clears the field (the frontend sends `null` when clearing dates or un-parenting). Project/section/group names are validated the same way (string, non-empty, ≤200 chars) and `reorder` requires a non-empty string section plus a string id list.
- **Timeouts & error hygiene**: every connection has a 30s socket timeout (slowloris guard); unexpected errors answer a JSON 500 — tracebacks go to the server log only, never to the client.
...
- **Watchdog**: `deploy/watchdog.sh` probes the origin (`/api/projects` with the Bearer token, restarts the app service) and the edge (DoH + `--resolve`; 200/401/302/303 count as healthy, restarts the tunnel) every 2 minutes — the 302/303 codes keep Cloudflare Access intercepts from being misread as outages. Known limitation: that also means the edge probe cannot distinguish "app broken behind a healthy Access intercept" from a real success (the 302 masks the app). A true end-to-end edge check needs a Cloudflare Access *service token* (`CF-Access-Client-Id`/`CF-Access-Client-Secret` headers) so the probe can pass the edge and hit the app for real.
...
POST   /api/projects/<pid>/tasks/bulk_offset  {"task_ids": [...], "days": N} shift start_on/due_on of many
                                              tasks by N days in one atomic write (tasks without start_on only
                                              move due_on; no dependency cascade); returns {"updated","missing","days"}
```
...
`backup_offsite.sh` packs `data/projects.json` + `data/projects/`, encrypts with AES-256-CBC (PBKDF2, 600k iterations, `openssl enc`) using the key in `data/backup_key.txt`, and drops the result into iCloud Drive (`~/Library/Mobile Documents/com~apple~CloudDocs/mini-asana-backups/`), keeping the latest 30 (pruning is done by an exact mtime-sorted unlink that tolerates the spaces in the iCloud path). With `local.miniasana-backup-offsite.plist` it runs daily at 04:23 (after the local 03:17 backup).

> **Caveat**: the script only *writes* into the local iCloud Drive folder — a successful run does NOT mean the file has finished uploading to iCloud. Confirm on a second Apple device (or icloud.com) that new backups actually appear there.
